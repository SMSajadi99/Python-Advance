...
# mycursor.execute("CREATE DATABASE info_car")
...
# res = requests.get('https://divar.ir/s/tehran/auto')
# # print(res.text)

# soup = BeautifulSoup(res.text,'html.parser')
# val = soup.find_all('div',attrs={'class':'kt-post-card__title'})
# val_Price = soup.find_all('div',attrs={'class':'kt-post-card__description'})


# # print(len(val) , len(val_Price))

# for i in range(len(val)):
#     # print(i)    

#     trash = type_persian(val[i].text).split()
#     trash_Price = type_persian(val_Price[i].text).split()

#     # print(trash)
#     # print(trash_Price)
#     # print(type_persian(val_Price[i].text))

#     reg = r'^(.*)$'
#     for j in (type_persian(val_Price[i].text).split()):
#         if (j == 'ﯼﻘﻔﺎﻮﺘ'):
#             # print('####')
#             name_care = trash[-1]
#             input_condition = type_persian('توافقی')

#             #####******************  DON'T EDITE **********
#             # print(trash[-1] , type_persian('توافقی'))
#             # str1 = ''.join(re.findall('ﯼﻘﻔﺎﻮﺘ',val_Price[i].text))
#             ####$$$$$$$$$$$$$$$$$$$$$$

#             values = (name_care,input_condition)
