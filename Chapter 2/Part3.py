...
class Player(Person):
    def random_team_A(self):
        return str(type_persian(self.man)) + ' ' + 'A'
    def  random_team_B(self):
        return str(type_persian(self.man)) + ' ' + 'B'

#################### The method
lst = ['حسین','مازیار','اکبر','نیما','مهدی','فرهاد','محمد','خشایار','میلاد','مصطفی','امین','سعید','پویا','پوریا','رضا','علی','بهزاد','سهیل','بهروز','شهروز','سامان','محسن']

for i in range(11):
    name_list = random.choice(lst)
    print(Player(name_list).random_team_A())
    lst.remove(name_list)
    # print(lst)
for i in lst:
    print(Player(i).random_team_B())
